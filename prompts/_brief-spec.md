Write my morning brief: the brief a good chief of staff would put on my desk. Three lenses (CEO, PA, Architect), three questions, options for the day and the people who went silent.
...
CEO LENS ("sections.ceo", 3 to 5 lines, at most 12): what matters today. Money in and out, deadlines, deals, threads that are blocked on someone. One line each in "title"; "body" only when the line needs context; "url" when you have a link; "priority" 1 (needs me today) to 5 (good to know).
...
ARCHITECT LENS ("sections.architect", at most 8): how my assistant setup could work better: a source that is not connected, a tool that failed, a step you could not do, a routine worth adding, a check that would have caught a miss. Empty is fine on a normal day.

3 QUESTIONS ("sections.questions", 0 to 3 strings): the three questions I should answer today, one line each, each answerable in a sentence. I answer them in the app; the answer comes back to you in the state as a user task titled "Q: ..." with my answer as its note.

OPTIONS FOR TODAY ("sections.options", exactly three keys): "deep_work" = if I have two hours of deep work, focus on this; "between_meetings" = if I am running between meetings, batch this; "exhausted" = if I am exhausted, do only this. One line each.

SILENT CONTACTS ("sections.silent", at most 20): people from email, calendar or messages I have not heard from or written to for more than 7 days while the thread is still open. "name", "days" since the last message, "last" = what the thread was about, "suggest" = one line I could send them today.

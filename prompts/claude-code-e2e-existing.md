...
## Step 4 - extend the existing routine

Find the user's existing routine (ask which one if there is more than one candidate). Add two steps to its task text, keeping everything it already does:
...
Run the extended routine once now. When the send command prints `sent brief seq=...`, tell the user the brief is in the app.

Confirm to the user in one line: routine extended, brief sent, nothing else stored.

## The rules to add to the routine
